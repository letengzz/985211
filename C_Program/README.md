...
​		在C语言程序中,可以用一个符号名来代表一个常量，称为符号常量。这个符号名必须在程序中进行特别的“指定”,并符合标识符的命名规则。如：`PI`
...
- #### 实型数据
  ...
  在内存中，实数一律以指数形式存放。**存在误差**
  
  **单精度**：数字后面加 "f"或"F" `2.3f` 
...
<img src="img/字符型变量.png" style="zoom:50%;"  >
