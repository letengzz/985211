...
**可屏蔽中断请求**(`Interrupt Request`)：([输入、高电平有效]())
...
**地址/状态分时复用引脚**(`Address`/`Status`)：([输出、三态]())
...
![image-20220426194504707](D:/Data/typora/photo/image-20220426194504707.png)

## <SPAN style="TEXT-DECORATION: overline">BHE</SPAN>/S<sub>7</sub>

**高8位数据总线允许/状态复用引脚**(`Bus High Enable`/`Status`)：([输出、三态、低电平有效]())
...
![image-20220427114818967](D:/Data/typora/photo/image-20220427114818967.png)

## MN/<SPAN style="TEXT-DECORATION: overline">MX</SPAN>

**最小/最大模式控制引脚**(`Minimum`/`Maximum` `Mode Control`)：([组态选择，输入]())

接**高电平**(+5V)时，8086引脚工作在[**最小模式**]()

接**低电平**(接地)时，8086引脚工作在[**最大模式**]()


## <SPAN style="TEXT-DECORATION: overline">RD</SPAN>

**读信号**(`Read`)：([输出、三态、低电平有效]())
...
**总线保持响应(总线响应)**(`HOLD Acknowledge`)：([输出，高电平有效]())

​	表示有其他设备向CPU请求使用总线。

------

**总线请求/允许信号**<SPAN style="TEXT-DECORATION: overline">RQ</SPAN>/<SPAN style="TEXT-DECORATION: overline">GT</SPAN><sub>0</sub>、<SPAN style="TEXT-DECORATION: overline">RQ</SPAN>/<SPAN style="TEXT-DECORATION: overline">GT</SPAN><sub>1</sub>：[双向]()
...
**总线请求响应信号**(`Hold Acknowledge`)：([输出，高电平有效]())。 
...
**写控制**(`Write`)：([输出、三态、低电平有效]())

​	表示当前CPU正在写存储器或I/O端口。

------

**总线封锁信号**(`LOCK`)：([输出、低电平有效]())。

​	当其为低电平时，系统中其它部件不得占有总线。

## M/ <SPAN style="TEXT-DECORATION: overline">IO</SPAN>(<SPAN style="TEXT-DECORATION: overline">S</SPAN><sub>2</sub>)

**I/O或存储器访问选择**(`Memory`/`Input and Output`)：([输出、三态]())

​	为高电平时，表示当前CPU正在访问[存储器]()
​	为低电平时，表示CPU当前正在访问[I/O端口](I/O端口)

------

**总线周期状态信号**<SPAN style="TEXT-DECORATION: overline">S</SPAN><sub>2</sub>、<SPAN style="TEXT-DECORATION: overline">S</SPAN><sub>1</sub>、<SPAN style="TEXT-DECORATION: overline">S</SPAN><sub>0</sub>：([输出]())。
...
**数据发送/接受控制信号**(`Data Transmit`/`Receive`)：(输出、三态)
...
**数据允许信号**(`Data Enable`)：([输出、三态、低电平有效]())
...
**地址锁存允许**(`Address Latch Enable`)：([输出、高电平有效]())

​	表示当前地址/数据分时使用的引脚上正在输出地址信号。是微处理器提供给地址锁存器[8282]()/[8283]()的控制信号(不能悬空)，**边沿触发**，由高到低的下降沿将其锁地址。

------

**指令队列状态信号**QS<sub>0</sub>、QS<sub>1</sub> ：([输出]())
...
**可屏蔽中断请求**(`Interrupt Acknowledge`)：([输出、三态、低电平有效]())
...
​	和[`WAIT`指令]()配合使用，执行`WAIT`时，CPU暂停执行程序，进入空转状态等待。当TEST引脚接收到一个低电平信号时，CPU就结束等待，继续向下执行指令。 和`WAIT`配合，可以实现CPU与外设同步工作。


## READY

**存储器或I/O口就绪**(`READY`)：([输入、高电平有效]())
...
![image-20220516155719372](D:/Data/typora/photo/image-20220516155719372.png)
...
5. <SPAN style="TEXT-DECORATION: overline">BHE</SPAN>/S<sub>7</sub>(`Bus High Enable`/`Status`)：**总线高字节有效信号**，[三态输出，低电平有效]()。

   ​      非数据传送期间，该引脚用作S<sub>7</sub>，输出状态信息。

6. ALE(`Address Latch Enable`)：**地址锁存允许信号**，[输出，高电平有效]()。

   ​      表示当前地址/数据分时使用的引脚上正在输出地址信号。向[地址锁存器8282]()提供地址锁存信号。

7. <SPAN style="TEXT-DECORATION: overline">DEN</SPAN>(`Data Enable`)：**数据允许信号**，[三态输出，低电平有效]()。表示当前地址/数据分时使用的引脚上正在传输数据信号。8286将它作为输出允许信号。进行DMA传输时，被置为高阻态。

8. DT/<SPAN style="TEXT-DECORATION: overline">R</SPAN>(`Data Transmit`/`Receive`)：**数据发送/接收控制信号**。[三态输出]()。当它为高电平时表示为数据发送， CPU写数据到存储器或I/O端口；为低电平时表示为数据接收，CPU从存储器或I/O端口读取数据。该信号用来控制总线收发器8286的数据传送方向。
...
![图片3](D:/Data/typora/photo/%E5%9B%BE%E7%89%873.png)
...
**I/O或存储器访问选择**(`Input and Output`/`Memory`)：([输出、三态]())

​	为高电平时，表示当前CPU正在访问[I/O端口](I/O端口)，这时地址总线A<sub>15</sub>~A<sub>0</sub>提供16位I/O口地址
​	为低电平时，表示CPU当前正在访问[存储器]()，这时地址总线A<sub>19</sub>~A<sub>0</sub>提供20位存储器地址
...
​	表示当前地址/数据分时使用的引脚上正在输出地址信号。是微处理器提供给地址锁存器[8282]()/[8283]()的控制信号(不能悬空)
...
1. IO/<SPAN style="TEXT-DECORATION: overline">M</SPAN>(`IO`/`Memory`)：**存储器或I/O端口访问选择信号**，[三态输出]()。
...
![image-20220515133003681](D:/Data/typora/photo/image-20220515133003681.png)
